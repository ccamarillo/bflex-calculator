...
class CalculatorTest extends TestCase {
    ...
    public function testInvalidValidateInputsThrowException() {
        ...
        $calculated = $calculator->calculate('test', 'something', 'else', 'entirely', 'bad', 'oh', 'my', 'sobad');
    }

    public function testValidInputs() {
        $calculator = new Calculator();
        $results = $calculator->calculate(100, 50, 265, 30, 2200, 'low', 7.23, 53);
        $this->assertEquals(get_class($calculator), 'Calculator');
    }

    public function testCalculateLow() {
        $calculator = new Calculator();
        $results = $calculator->calculate(1000, 750, 265, 30, 2200, 'low', 7.23, 53);;
